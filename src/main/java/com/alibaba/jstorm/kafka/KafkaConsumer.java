...
public class KafkaConsumer {
    ...
    // 指的是partition未被消费；offset默认为-1
    public static final int NO_OFFSET = -1;
    // 当前consumer的状态
    private int status;
    // consumer
    private SimpleConsumer consumer = null;
    // 对应kafka的producer相关配置
    private KafkaSpoutConfig config;
    // 当前consumer消费partition所在的broker
    private LinkedList<Host> brokerList;
    // 当前正在被消费的broker index
    private int brokerIndex;
    // 在同一时间 同一个topic的多个partition只有一个被消费
    ...
    // 根据指定的partition 和 当前partition被消费的offset 消费对应的topic的message
